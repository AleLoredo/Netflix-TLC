...
# 2. ESTIMAR PARÁMETROS POBLACIONALES DE FORMA EFICIENTE
print("2. ESTIMACIÓN RÁPIDA DE PARÁMETROS POBLACIONALES:")

# Tomar una muestra grande para estimar μ y σ (solo una vez)
estimation_fraction = min(0.01, 100000 / total_count)  # Máximo 1% o 100K registros
...
print(f"Muestra de estimación: {estimation_sample.count():,} registros")
...
# 4. FUNCIÓN EFICIENTE PARA GENERAR MUESTRAS
def get_clean_sample(df_source, target_size, max_attempts=5, seed_base=None):
    """
    Genera una muestra limpia de tamaño exacto target_size
    usando la estrategia sample-first, clean-later
    """
    # Calcular fracción inicial considerando el ratio de limpieza
    # Multiplicamos por factor de seguridad para compensar pérdidas
    safety_factor = 2.0
    initial_fraction = (target_size * safety_factor) / (total_count * clean_ratio)
    
    for attempt in range(max_attempts):
        # Generar seed único para cada intento
        current_seed = (seed_base * 1000 + attempt) if seed_base else None
        
        # 1. SAMPLE FIRST - tomar muestra cruda
        ...
        # 2. CLEAN LATER - aplicar filtros solo a la muestra
        ...
        if sample_count >= target_size:
            ...
            if sample_count > 0:
                ...
                initial_fraction = (target_size * 1.5) / (total_count * actual_ratio)
            else:
                initial_fraction *= 2  # Duplicar si no obtuvimos datos válidos
    
    # Si llegamos aquí, usar lo que pudimos obtener
    return clean_sample

# 5. GENERAR MUESTRAS DE FORMA EFICIENTE
print("4. GENERANDO MUESTRAS CON ESTRATEGIA EFICIENTE...")
...
for i in range(n_samples):
    try:
        # Generar muestra limpia eficientemente
        sample_df = get_clean_sample(df_raw, n, seed_base=i)
